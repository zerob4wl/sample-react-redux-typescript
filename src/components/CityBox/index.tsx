...
@(connect(
  mapStateToProps,
  mapDispatchToProps
) as any) // TODO: should fixed after typescript issue resolved
export default class CityBox extends React.Component<IProps, IState> {
  ...
  public componentDidMount() {
    ...
    const api = new API();
    api.getLocation({ woeid: this.props.city.woeid }).then((result: IGetLocationResult) => {
      if (locationInfo) {
    ...
    return (
      <div className="city-box">
        ...
        {this.state.locationInfo && (
          <div>
            <h4>
              {this.state.locationInfo.consolidated_weather[0].the_temp}
              <small>°C</small>
            </h4>
        ...
        <div className="spinner">
          <BarLoader color={"#ff6369"} loading={this.state.loading} />
          <BarLoader color={"#fefcf3"} loading={this.state.reload} />
        </div>
